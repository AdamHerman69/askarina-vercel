...
const fragShaderOutput = /* glsl */ `
    ...
    void main () {
        ...
        if (u_chromatic_aberration > 0.0) {
            ...
            float r = texture2D(u_image_texture, redUV).r;
            float g = texture2D(u_image_texture, greenUV).g;
            float b = texture2D(u_image_texture, blueUV).b;
            float a = texture2D(u_image_texture, greenUV).a;
            
            gl_FragColor = vec4(r, g, b, a);
        } else {
            vec2 displacedUV = clamp(baseUV + displacement, 0.0, 1.0);
            gl_FragColor = texture2D(u_image_texture, displacedUV);
        }
    }
`

export type FluidImageOptions = {
    imageSrc: string
...
function FluidImageScene({ options, imageTexture }: { options: Required<FluidImageOptions>, imageTexture: THREE.Texture }) {
    ...
    const materials = useMemo(() => {
        return {
            ...
            output: new THREE.ShaderMaterial({
                uniforms: {
                    u_velocity_texture: { value: null },
                    u_image_texture: { value: imageTexture },
                    u_texel: { value: new THREE.Vector2(1/simWidth, 1/simHeight) },
    ...
    }, [simWidth, simHeight, options.velocityDissipation, options.displacementScale, options.chromaticAberration, imageTexture])
    ...
    useEffect(() => {
        ...
        materials.output.uniforms.u_image_texture.value = imageTexture
    }, [materials, options.velocityDissipation, options.displacementScale, options.chromaticAberration, imageTexture])
...
function FluidImageInner({ options }: { options: Required<FluidImageOptions> }) {
    const texture = useTexture(options.imageSrc)
    
    return <FluidImageScene options={options} imageTexture={texture} />
}
...
export default function FluidImage({ 
    ...
    const options: Required<FluidImageOptions> = {
        imageSrc,
        velocityDissipation,
        pressureIterations,
        splatRadius,
    ...
    return (
        <>
            <FluidImageInner options={options} />
